...
/**
 * Generated class for the PublicUserSetMobilePage page.
 *
...
@IonicPage()
@Component({
  selector: 'page-public-user-set-mobile',
  templateUrl: 'public-user-set-mobile.html',
})
export class PublicUserSetMobilePage {
  ...
  getImgCode() {
    ...
    this.mainCtrl.httpService.get(this.mainCtrl.httpService.config.host.org + 'v2/user/getImageCode', { uuid: this.mainCtrl.httpService.config.uuid }).subscribe(data => {
      if (data.success) {
  ...
  getPhoneCode() {
    if (this.modifyMobileData.imgCode == '' || this.modifyMobileData.phone == '') {
      ...
      //图片验证码是否正确
      if (!this.imgCodeFlag) {
        if (!this.isend && this.mainCtrl.commonModel.interva.modifyMobile == 60) {
          this.isend = true;
          this.mainCtrl.httpService.get(this.mainCtrl.httpService.config.host.org + 'v2/user/getPhoneCode1', {
            ...
            if (data.success) {
              ...
              this.timeInterval = setInterval(() => {
                if (this.mainCtrl.commonModel.interva.modifyMobile != 0) {
                  this.mainCtrl.commonModel.interva.modifyMobile--;
                } else {
                  this.mainCtrl.commonModel.interva.modifyMobile = 60;
                  this.isend = false;
                  clearInterval(this.timeInterval);
                }
              }, 1000);
  ...
  modifyMobule() {
    ...
    this.mainCtrl.httpService.post(this.mainCtrl.httpService.config.host.org + url).subscribe(data => {
      if (data.success) {
        this.mainCtrl.commonModel.TAB_INIT_USERINFO.mobile = this.modifyMobileData.phone;
        this.mainCtrl.nativeService.showToast(data.msg);
        setTimeout(() => {
          this.navCtrl.pop();
        }, 500);
      } else {
        this.mainCtrl.nativeService.showToast(data.msg || '连接失败')
      }
    })
  }
  ...
  ionViewDidEnter() {
    this.mainCtrl.httpService.get(this.mainCtrl.httpService.config.host.org + 'user/userinfo').subscribe(data => {
      if (data.success) {
